...
//    println(invoice)
  ...
  private fun calculateInvoiceDetails(mapConfig: Map<Int, Long>, price: BigDecimal, year: Int, month: Int): Flux<InvoiceDetail> {
    ...
    for (i in 1..totalDays) {
      ...
      invoiceDetails.add(InvoiceDetail(date = i, dayInWord = WeekDays.new(tempStartDay), milkPerDay = milkPerDay.toInt(), totalCostPerDay = pricePerDay))
  ...
  fun dummyInvoiceDetails(): List<InvoiceDetail> {
    return ObjectMapper().registerModule(KotlinModule()).readValue<List<InvoiceDetail>>(File("src/test/resources/InvoiceDetails.json"))

  }
}

data class InvoiceDetail(val date: Int, val dayInWord: WeekDays, val milkPerDay: Int, val totalCostPerDay: BigDecimal)

data class Invoice(val name: String, val month: Int, val year: Int, val vendorName: String, val billingDate: LocalDate, val descriptions: List<InvoiceDetail>)
